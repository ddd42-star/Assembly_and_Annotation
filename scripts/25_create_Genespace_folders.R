...
# Load the annotation
annotation <- fread("/data/users/dbassi/assembly_and_annotation-course/maker/final/filtered.genes.renamed.final.gff3", header = FALSE, sep = "\t")
...
top20_scaff <- fread("/data/users/dbassi/assembly_and_annotation-course/assemblies/flye-assembly/assembly.fasta.fai", header = FALSE, sep = "\t") %>%
    select(V1, V2) %>%
    arrange(desc(V2)) %>%
    head(20)
...
longest_proteins <- "/data/users/dbassi/assembly_and_annotation-course/maker/final/longest_proteins.fasta"
